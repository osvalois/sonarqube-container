...
if [ -f railway.toml ]; then
    POSTGRES_URL=$(grep "SONAR_JDBC_URL" railway.toml | cut -d '=' -f2 | tr -d ' "')
    POSTGRES_USER=$(grep "SONAR_JDBC_USERNAME" railway.toml | cut -d '=' -f2 | tr -d ' "')
    POSTGRES_PASSWORD=$(grep "SONAR_JDBC_PASSWORD" railway.toml | cut -d '=' -f2 | tr -d ' "')
...
echo "==========================================="
echo "Now you can redeploy SonarQube to Railway with:"
echo "railway up"
echo "==========================================="